...
export async function getTxEventData(
    ...
    const tx = (
        input && Object.prototype.toString.call(input) === '[object Promise]' ? await input : input
    ) as ContractTransaction;
    const { logs } = await tx.wait();
    const idx = _idx ?? logs.length - 1;
    return contractInterface.decodeEventLog(eventFragment, logs[idx].data, logs[idx].topics);
}

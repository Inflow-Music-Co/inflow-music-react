...
const ManageLabelArtists = () => {
    ...
    const getlabelartists = async () => {
        try {
            ...
            const { data } = await axios.post(
                `${process.env.REACT_APP_SERVER_URL}/v1/label/getlabelartists`,
                { labelid }
            );
            // console.log({data})
    ...
    const removelabelartist = async (e) => {
        try {
            ...
            await axios.post(`${process.env.REACT_APP_SERVER_URL}/v1/label/removeartist`, {
                labelid,
                artistid
            });
            ...
            return (
                <tr key={i}>
                    ...
                    <td className="text-center">
                        {artist.first_name
                            ? `${artist.first_name} ${artist.last_name ? artist.last_name : ''}`
                            : artist.name}
                    </td>
                    <td className="text-center">
                        <Button variant="danger" id={artist._id} onClick={removelabelartist}>
                            Remove
                        </Button>
                    </td>
                </tr>
    ...
    const addArtistToLabel = async () => {
        try {
            ...
            setloading(true);
            await axios.post(`${process.env.REACT_APP_SERVER_URL}/v1/label/addartist`, {
                labelid,
                artistid: sartistid
            });
            await getlabelartists();
    ...
    return (
        <div className="dashboard-wrapper-main vw-100" style={{ minHeight: '100vh' }}>
            <div className="row mb-4 ml-2">
                <Button variant="primary" onClick={() => setaddmodal((addmodal) => !addmodal)}>
                    Add Artist
                </Button>
            </div>
            <Table striped bordered hover variant="dark">
                <thead>
                    <tr>
                        <th className="text-center">Social Token</th>
                        <th className="text-center">Artist Name</th>
                        <th className="text-center">Remove Artist</th>
                    </tr>
                </thead>
            ...
            <Modal
                style={{ color: '#000' }}
                show={addmodal}
                onHide={() => setaddmodal((addmodal) => !addmodal)}
            >
                ...
                <Modal.Body>
                    <select
                        name="addartists"
                        id="addartists"
                        onChange={(e) => setsartistid(e.target.value)}
                    >
                ...
                <Modal.Footer>
                    <Button variant="primary" onClick={addArtistToLabel}>
                        Add Artist
                    </Button>
                </Modal.Footer>
            </Modal>

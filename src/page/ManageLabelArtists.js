...
const ManageLabelArtists = () => {
  ...
  const removelabelartist = async (e) => {
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_SERVER_URL}/v1/label/removeartist`,
        { labelid, artistid }
      );
  ...
  const addArtistToLabel = async () => {
    try {
      ...
      setloading(true);
      await axios.post(
        `${process.env.REACT_APP_SERVER_URL}/v1/label/addartist`,
        { labelid, artistid: sartistid }
      );
      await getlabelartists();
      setloading(false);
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="row mb-4 ml-2">
        <Button
          variant="primary"
          onClick={() => setaddmodal((addmodal) => !addmodal)}
        >
          Add Artist
        </Button>
      </div>

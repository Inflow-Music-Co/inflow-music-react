...
import { inflowAddressesByChainId, getAddressesByChainId, inflowABIs, getAbi } from './constants';
import {
    ChainId,
    Contracts,
    FormattedAndBNTuple,
...
export class Inflow {
    ...
    constructor(provider: providers.Provider, chainId?: ChainId) {
        ...
        this.addresses = getAddressesByChainId(inflowAddressesByChainId, this.chainId);
    ...
    async getRoyaltiesPaid(tokenId: BigNumberish, next?: boolean): Promise<BigNumber[]> {
    ...
    async getToken(tokenType: ERC1155, tokenId: BigNumberish): Promise<Token1155> {
        const contract = tokenType === 'Inflow1155BC' ? this.inflow1155BC : this.inflow1155;
    ...
    async isApprovedForAll(tokenType: NFT, account: string, operator: string): Promise<boolean> {
        const contract = this.getNFTContract(tokenType);
    ...
    async getTokenSocialFactory(creator: string): Promise<string> {
        ...
        const socialTokenAddress = await this.socialTokenFactory.getToken(creator);
    ...
    async totalSupply(tokenType: ERC20, socialTokenAddress?: string): Promise<FormattedAndBNTuple> {
        this.checkSocialToken(tokenType, socialTokenAddress);
        const contract =
            tokenType === 'SocialToken'
                ? (new Contract(
    ...
    async isWhitelisted(tokenType: WhitelistEnabled, account: string): Promise<boolean> {
        const contract =
    ...
    async storeDirectory(storeDirectoryData: StoreDirectoryData[]): Promise<CIDString> {
        if (this.ipfsClient === undefined) {
            throw new Error('IPFS client uninitialized');
        }
        const data = storeDirectoryData.map(
            ({ fileBits, fileNames, options }) => new File(fileBits, fileNames, options)
        );
    ...
    formatERC20(tokenType: ERC20, amount: BigNumber): string {
        return tokenType === 'SocialToken' ? utils.formatEther(amount) : this.formatUsdc(amount);
    }

    parseERC20(tokenType: ERC20, amount: string): BigNumber {
        return tokenType === 'SocialToken' ? utils.parseEther(amount) : this.parseUsdc(amount);
    }
